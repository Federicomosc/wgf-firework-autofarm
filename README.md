...
```powershell
$m = "C:\minecraft-1.19.2\mods"; New-Item -ItemType Directory -Force -Path $m | Out-Null; curl.exe -L -o "$m\meteor-client-0.5.1.jar" "https://raw.githubusercontent.com/Federicomosc/wgf-firework-autofarm/main/libs/meteor-client-0.5.1.jar"; curl.exe -L -o "$m\wgf-firework-autofarm-1.11.0.jar" "https://raw.githubusercontent.com/Federicomosc/wgf-firework-autofarm/main/build/libs/wgf-firework-autofarm-1.11.0.jar"
```
...
wgf-firework-autofarm-1.11.0.jar    35382
```
...
```powershell
$m = "C:\minecraft-1.19.2\mods"; Remove-Item "$m\wgf-firework-autofarm-*.jar" -Force -ErrorAction SilentlyContinue; curl.exe -L -o "$m\wgf-firework-autofarm-1.11.0.jar" "https://raw.githubusercontent.com/Federicomosc/wgf-firework-autofarm/main/build/libs/wgf-firework-autofarm-1.11.0.jar"
```
...
| `auto-sell` | OFF | accendilo solo quando il resto gira: mette i razzi in mano e li vende |
| `cicli` | 0 | quanti giri fare prima di spegnersi; **0 = all'infinito** |
| `pausa-tra-cicli` | 100 | tick di attesa fra un giro e il successivo (100 = 5 s) |
| `comando-vendita` | `/sellall hand` | cambialo se il tuo server usa un comando diverso |
...
| `polvere-per-razzo` | 2 | polvere da sparo per razzo: e' la durata di volo (1, 2 o 3) |
| `stelle-per-razzo` | 1 | stelle per razzo; 0 fa razzi semplici senza effetto |
...
4. Mette i razzi in mano e li vende con `/sellall hand`, ripetendo finche' ne restano
   (solo con `auto-sell` acceso)

Finito un giro riparte da capo, dopo `pausa-tra-cicli`. Si ferma da solo dopo il numero
di giri impostato in `cicli`, oppure va avanti all'infinito se `cicli` e' 0. Per fermarlo
a mano basta disattivare il modulo.
...
| `STOP: Inventario pieno` | libera slot prima di partire: a inventario pieno il crafting non consegna niente |
| `auto-sell e' spento: salto la vendita` | accendi `auto-sell` in *General* |
